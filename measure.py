...
def append_log(msg):
    fn = datetime.now().astimezone(pytz.timezone('Asia/Seoul')).strftime("%Y%m%d.log")
    now = datetime.now().astimezone(pytz.timezone('Asia/Seoul')).strftime("%Y%m%d %H:%M:%S")
    current_directory = os.getcwd()
    fn = os.path.join(current_directory, 'data', '%s' % fn)
...
def create_csv(data,mode):
    fn0 = datetime.now().astimezone(pytz.timezone('Asia/Seoul')).strftime("%Y%m%d_%H%M%S.csv")
    current_directory = os.getcwd()
    fn = os.path.join(current_directory, 'data', '%s_%s' % (mode, fn0))
...
def main():
    ...
    # Design of the Setting Window at Frame3 for the log message
    ...
    def update_data(mode):
        ...
        if mode == "VI":
            ...
            for vg in Vgs:
                ...
                inst.write('F2A4B1RA1I1L2M3\n')
                inst.write(comm_sweep)
                inst.write('W1\n')

                while True:
                    val = inst.read_bytes(23 * 1).decode('ascii')
            ...
            msg = '%s : %s' % (csv, param['msg_IV'])
            print(append_log(msg))
        ...
        if mode == "Z":
            ...
            inst.write('VA ' + str(vac) + '\n')
            inst.write('VB ' + str(vdc) + '\n')
            inst.write('SW 0\n')

            for freq in freqs:
                # configure generator output
                inst.write('FR ' + str(freq) + '\n')
...
#                time.sleep(2)
                ...
                output = inst.read_raw().decode('ascii')
                print(output)
                output = output.split(',')
            ...
            csv = create_csv(data_json,'Z')
            msg = '%s : %s' % (csv, param['msg_Z'])
            print(append_log(msg))
    ...
    def check_variable():
        ...
        param = { 'StartVg': float(startvg_val.get()), 'StopVg': float(stopvg_val.get()), \
             ...
             'G_logscale': int(checkgoption.get()), 'DelayD': float(delayd_val.get()),\
             'DelayG': float(delayg_val.get()),\
             ...
             'msg': memo_Text.get("1.0","end"), 'msg_IV': memo_Text_IV.get("1.0","end"),\
             'msg_Z': memo_Text_Z.get("1.0","end") }
    ...
    memo_Text_IV = tkinter.Text(frame1,height=2,width=17)
    memo_Text_IV.grid(row=16,column=0,columnspan=2,sticky=tkinter.N+tkinter.W)

    SaveIV_Button = tkinter.Button(frame1,text="Save",command=lambda: save_memo("1"),width=15,overrelief="solid",repeatdelay=1000,repeatinterval=100)
    SaveIV_Button.grid(row=17,column=0,columnspan=2,sticky=tkinter.N+tkinter.W)
    ...
    memo_Text_Z = tkinter.Text(frame2,height=2,width=17)
    memo_Text_Z.grid(row=11,column=0,columnspan=2,sticky=tkinter.N+tkinter.W)

    SaveZ_Button = tkinter.Button(frame2,text="Save",command=lambda: save_memo("2"),width=15,overrelief="solid",repeatdelay=1000,repeatinterval=100)
    SaveZ_Button.grid(row=12,column=0,columnspan=2,sticky=tkinter.N+tkinter.W)


    # Frame 3

    memo_Text = tkinter.Text(frame3,height=2,width=17)
    memo_Text.grid(row=1,column=0,columnspan=2,sticky=tkinter.N+tkinter.W)

    Check_Button = tkinter.Button(frame3,text="Save the memo",command=lambda: save_memo("3"),overrelief="solid",repeatdelay=1000,repeatinterval=100)
    Check_Button.grid(row=2,column=0,columnspan=2,sticky=tkinter.N+tkinter.W)
